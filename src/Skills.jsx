...
const skills = [
  {
    key: "01", icon: ICONS.html, name: "HTML", level: 95,
  ...
  {
    key: "02", icon: ICONS.css, name: "CSS", level: 90,
  ...
  {
    key: "03", icon: ICONS.js, name: "JavaScript", level: 85,
  ...
  {
    key: "04", icon: ICONS.react, name: "React", level: 88,
  ...
  {
    key: "05", icon: ICONS.figma, name: "Figma", level: 80,
...
function SkillCard({ skill, index, sectionInView, isTouch }) {
  ...
  useEffect(() => {
    ...
    let raf;
    const loop = () => {
      if (cardRef.current && isHovering.current) {
        cardRef.current.style.transform = `perspective(900px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)`;
  ...
  return (
      <div
        ...
        role="button"
        tabIndex={0}
        aria-label={`${skill.name}: ${flipped ? skill.flip : skill.description}`}
      >
      <div className={`skill__inner${!isTouch && flipped ? " skill__inner--flipped" : ""}${isTouch ? "" : ""}`}>
  ...
  return (
    <section id="skills" className="skills" ref={sectionRef}>
      ...
      <div className={`skillHeading${inView ? " skillHeading--visible" : ""}`}>
        <h2><ScrambleText text="Arsenal" /></h2>
        <p>
